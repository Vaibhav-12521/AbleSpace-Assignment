...
| 3, 7 — Fields popover               | toolbar on any task view     |
| 4 — List                            | `/tasks` (list mode)         |
| 5 — Search                          | toolbar, `⌘F` / `Ctrl+F`     |
| 6, 8 — Task detail + date picker    | `/tasks/[id]`                |
| 9, 10 — Account, Theme, Color menus | sidebar workspace switcher   |
| 11 — Filter menu                    | toolbar on any task view     |
| 12 — Project task list              | `/projects/[id]`             |
| 13 — Settings                       | `/settings`                  |

**Beyond the static frames:** guest login with JWT sessions, drag-and-drop
board reordering with optimistic updates, inline creation of tasks / subtasks /
projects, editable task properties, threaded comments, an activity feed,
persisted theme + accent, and per-screen persisted view preferences.

---

## Design fidelity

Colours and sizing were **sampled from the PNG exports**, not estimated. A
...
script that runs **before first paint**, so a reload never flashes the wrong
theme. React reads the same values through `useSyncExternalStore`, which also
means changing the theme in one tab updates every other open tab.
...
groups: inconsistencies **within the Figma file**, and things the Figma
**doesn't cover**.
...
4. **Fields toggles contradict what the frames render.** Screen 3 shows Labels
   *off*, yet board cards draw label chips; the list view has no Labels column
   at all. Field visibility is therefore stored **per view mode**, with each
...
6. **The projects primary button is labelled inconsistently** — "Add Project"
   on screen 9, "Add Task" on screens 10 and 11. Uses "Add Project", which is
   what it does.
7. **Screen 6's Details rail shows status "Backlog"**, which is not one of the
   board's columns (To Do / Doing / Completed / On Hold). The rail shows the
...
8. **No dark-theme frames exist.** All 13 exports are light. The dark palette
   ...
   the neutral scale. Happy to match real dark frames if they're available.
9. **No tablet or mobile frames exist.** Responsive behaviour is my own:
   ...
   than scrolling a five-column table sideways; the task detail rail stacks
...
12. **Google login is rendered but disabled.** Only Guest Login is in scope;
    real OAuth needs credentials and a consent screen. It carries a tooltip
    explaining why — a button that looks live but does nothing would be worse.
13. **Due-date filter boundaries use the server's local midnight.** A
...
apps/
  ...
  web/                    Next.js
    src/
      app/                routes; (app) group is auth-guarded
      ...
      lib/                api client, tokens, theme, dates
```
...
- **Reordering runs in a transaction.** `move()` renumbers siblings and updates
  the moved task atomically, so a failure can't leave two cards on one
  position.
- **`TaskWorkspace` is shared** by `/tasks` and `/projects/[id]` — the same
  screen at different scopes, so both routes are a three-line page.
- **The calendar is hand-rolled.** The design only needs single-date selection;
  this drops a dependency and keeps the markup exact.
...
`assigneeIds`, `labelIds`, `teamIds` (comma-separated) and `due`
...
Open `http://localhost:3000`, click **Continue as Guest**, and you land in a
...
- **Web** — Vercel, root directory `apps/web`, with `NEXT_PUBLIC_API_URL` set
  to the Render URL.

> **Free-tier caveat:** a Render free web service spins down after ~15 minutes
> idle and takes 30–60s to cold start. An uptime pinger hitting
> `/api/health` every 10 minutes keeps it warm for the 45-day review window.
...
- **`tasks.service.spec.ts`** — the position arithmetic in `move()`, which is
  the riskiest logic here and would corrupt board order if it were subtly
  wrong. Covers reordering up and down within a column, the gap-close /
...
- **`create-task.dto.spec.ts`** — DTO validation: empty titles, missing
...
- Test coverage stops at those two units. Controller and integration tests
  against a real database are the next thing I'd add.
- Board **column** reordering isn't wired up — the grip renders, and the API
...
- Real-time collaboration (the presence cursor in screen 2) is not implemented.
